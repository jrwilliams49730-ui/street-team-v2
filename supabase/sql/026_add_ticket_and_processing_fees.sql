-- Street Team V2: add Street Team fee + Stripe processing fee to paid reservations

alter table public.ticket_reservations
add column if not exists ticket_subtotal_cents_snapshot integer not null default 0
  check (ticket_subtotal_cents_snapshot >= 0),
add column if not exists street_team_fee_cents_snapshot integer not null default 0
  check (street_team_fee_cents_snapshot >= 0),
add column if not exists processing_fee_cents_snapshot integer not null default 0
  check (processing_fee_cents_snapshot >= 0);

update public.ticket_reservations
set
  ticket_subtotal_cents_snapshot = total_price_cents_snapshot,
  street_team_fee_cents_snapshot = 0,
  processing_fee_cents_snapshot = 0
where ticket_subtotal_cents_snapshot = 0
  and total_price_cents_snapshot > 0;
