-- Street Team V2: producer/venue box office ticket sales
...
begin
  alter table public.ticket_reservations
  add constraint ticket_reservations_sales_channel_check
  check (sales_channel in ('online', 'door'));
exception
  when duplicate_object then null;
end $$;

create index if not exists ticket_reservations_event_sales_channel_idx
on public.ticket_reservations (event_id, sales_channel, created_at desc);
...
create or replace function public.cancel_door_ticket_reservation(
...
begin
  ...
  if v_user_id is null then
    raise exception 'You must be signed in to cancel a box office sale.';
  end if;
  ...
  if v_reservation.sales_channel <> 'door' then
    raise exception 'Only box office reservations can be cancelled here.';
  end if;
  ...
  if not exists (
    ...
    where events.id = v_reservation.event_id
      and events.owner_user_id = v_user_id
  ) then
    raise exception 'Only the event owner can cancel this box office sale.';
  end if;
  ...
  if v_reservation.reservation_status = 'pending' then
    update public.ticket_reservations
    set
...
grant execute on function public.cancel_door_ticket_reservation(uuid)
to authenticated;
